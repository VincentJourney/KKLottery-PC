			...
			try
			{
				if (!IsPostBack)
				{
					...
					if (string.IsNullOrWhiteSpace(UnionId))
					{
						...
						{
							string url = $@"https://open.weixin.qq.com/connect/oauth2/authorize?appid={Setting.AppID}&redirect_uri=https%3a%2f%2fcloud.ascentis.cn%2fKKLottery%2fTurn-WC&response_type=code&scope=snsapi_userinfo&state=STATE#wechat_redirect";
